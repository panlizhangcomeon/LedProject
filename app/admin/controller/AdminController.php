    ...
    public function addSubmit()
    {
    	if($this->request->isPost()){
            ...
            if ($result !== true) {
				...
				Db::startTrans();
				try{
				    $uid = Loader::model('Admin')->strict(false)->field(true)->insertGetId($param);
					...
					Loader::model('AdminGroupAccess')->strict(false)->field(true)->insertAll($data);
				    // 提交事务
				    Db::commit();    
				} catch (\Exception $e) {
				    // 回滚事务
    ...
    public function editSubmit()
    {
        if($this->request->isPost()){
            ...
            if ($result !== true) {
                ...
                Db::startTrans();
                try{
                    Loader::model('Admin')->where(['id'=>$param['id']])->strict(false)->field(true)->update($param);
